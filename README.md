...
- 日別の出動地点を地図上にプロット
- 曜日別に色分けして可視化
- **🎬 アニメーション表示**: 1日の出動を時系列で再生（デモ映え◎）

### ⭐ 配置最適化
- **リソースを考慮した最適配置シミュレーション**
- 出動密度とカバレッジギャップから候補地点を自動抽出
- 新規消防署設置時の改善効果を数値で確認
- 高速な貪欲法アルゴリズム（デモンストレーション向け）
...
├── optimization.py           # 配置最適化モジュール（NEW）
├── traffic_analysis.py       # 渋滞考慮の到達圏計算
...
│   └── optimization_candidates.json  # 最適化候補地点キャッシュ
├── misc/                     # 実験用・分析スクリプト
│   ├── coverage_analysis.py  # リソースベースカバレッジ分析
│   ├── learn_delays.py       # 遅延パターン学習
│   ├── experiment.py
...
### タブ: 配置最適化（⭐NEW）
- **設定**: 候補地点数、到達時間閾値、新規救急車台数を指定
- **候補地点の自動抽出**: 出動密度とカバレッジギャップから最適候補を生成
- **シミュレーション結果**: 新規カバー件数、効率スコア（件/台）を表示
- **マップ表示**: 既存署、候補地点、出動地点を可視化
- **キャッシュ対応**: 2回目以降は高速に実行可能（デモ向け）
...
- **最適化**: scipy（KDTree）、貪欲法アルゴリズム

## 更新履歴

- **2026/02/02**: 出動アニメーション機能追加、リソース考慮の配置最適化機能を追加
